...
- Für ganze Aufgaben (Modellaufbau, Zeichnungsableitung, Export) zuerst `skills/nx-live-scripting/references/verified-recipes.md` lesen — dort stehen die gelaufenen Sequenzen **und** die Wege, die nachweislich nicht funktionieren. Erste Regel dort: mehrere Builder haben `Tolerance = 0.0` als Vorgabe, committen klaglos und scheitern erst später (Feature lässt sich nicht wieder öffnen, oder eine Fehlermeldung, die etwas anderes benennt).
